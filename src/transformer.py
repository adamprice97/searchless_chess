...
def build_param_action_decoder(config: TransformerConfig):
  """Stateless AR decoder. apply(params, rng, state_tokens, greedy=..., temperature=...)."""
  def forward(state_tokens: jax.Array, *, greedy: bool = True, temperature: float | None = None):
    # Fetch the RNG that Haiku received at apply(..., rng, ...)
    rng = hk.next_rng_key()
